...
async function inicializarDB() {
    try {
        ...
        try { await db.execute(`ALTER TABLE gastos_socios ADD COLUMN implicados TEXT DEFAULT 'Giancarlo,David,Kevin'`); } catch (e) { } // Nueva columna
...
// --- API USUARIOS & CLIENTES & OTS (Comprimidas) ---
...
app.post('/api/gastos', async (req, res) => {
    ...
    const impStr = Array.isArray(implicados) ? implicados.join(',') : 'Giancarlo,David,Kevin';
    
    try { 
        await db.execute({ sql: `INSERT INTO gastos_socios (pagador, concepto, importe, fecha, implicados) VALUES (?, ?, ?, ?, ?)`, args: [pagador, concepto, parseFloat(importe), fecha, impStr] }); 
...
app.delete('/api/gastos/:id', async (req, res) => {
    if (req.headers['x-rol'] !== 'admin' && req.headers['x-rol'] !== 'director') return res.status(403).json({ error: 'Sin permisos' });
    try { await db.execute({ sql: `DELETE FROM gastos_socios WHERE id = ?`, args: [req.params.id] }); res.json({ mensaje: 'Gasto eliminado.' }); } 
    catch (e) { res.status(500).json({ error: e.message }); }
});
