...
async function inicializarDB() {
    try {
        ...
        // --- NUEVA TABLA: GASTOS SOCIOS (SPLITWISE) ---
        ...
        console.log('✅ Base de datos Turso conectada y 100% operativa (Con Módulo de Gastos).');
...
app.put('/api/logs/:id/resolver', async (req, res) => {
    ...
    try {
        ...
        if (log.accion === 'Añadir OT') { const estado = datos.fecha_completada ? 'HECHO' : 'PENDIENTE'; await db.execute({ sql: `INSERT INTO ordenes_trabajo (codigo_ot, fecha_encargo, fecha_completada, horas, num_tecnicos, marca, tipo_urgencia, materiales_precio, estado, cliente_id, tecnicos_nombres) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, args: [datos.codigo_ot, datos.fecha_encargo, datos.fecha_completada || null, datos.horas, datos.num_tecnicos, datos.marca, datos.tipo_urgencia, datos.materiales_precio, estado, datos.cliente_id || null, datos.tecnicos_nombres || ''] });
...
app.post('/api/test-email', async (req, res) => {
    ...
    try {
        const payload = { to: emailDestino, subject: "🛠️ Prueba de conexión - ServiPlusUltra", html: `<div style="text-align: center;"><h2 style="color: #1abc9c;">¡El túnel secreto funciona! 🚀</h2></div>` };
    ...
    } catch (error) { res.status(500).json({ error: 'Fallo al enviar el correo por el puente.' }); }
});

app.post('/api/enviar-factura', async (req, res) => {
    const { emailDestino, asunto, htmlBody, pdfBase64, nombreArchivo } = req.body;
    try {
        const payload = { to: emailDestino, subject: asunto, html: htmlBody, adjuntoBase64: pdfBase64, adjuntoNombre: nombreArchivo };
        await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', body: JSON.stringify(payload) });
        res.json({ mensaje: 'Factura enviada con éxito al cliente por correo electrónico. 🚀' });
    } catch (error) { console.error(error); res.status(500).json({ error: 'Fallo de conexión al enviar la factura.' }); }
});

// --- NUEVA API: GASTOS SOCIOS (SPLITWISE) ---
app.get('/api/gastos', async (req, res) => {
    if (req.headers['x-rol'] !== 'admin' && req.headers['x-rol'] !== 'director') return res.status(403).json({ error: 'Sin permisos' });
    try { const r = await db.execute("SELECT * FROM gastos_socios ORDER BY id DESC"); res.json(r.rows); } 
    catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/gastos', async (req, res) => {
    if (req.headers['x-rol'] !== 'admin' && req.headers['x-rol'] !== 'director') return res.status(403).json({ error: 'Sin permisos' });
    const { pagador, concepto, importe } = req.body; const fecha = new Date().toLocaleString('es-ES');
    try { 
        await db.execute({ sql: `INSERT INTO gastos_socios (pagador, concepto, importe, fecha) VALUES (?, ?, ?, ?)`, args: [pagador, concepto, parseFloat(importe), fecha] }); 
...
app.delete('/api/gastos/:id', async (req, res) => {
    if (req.headers['x-rol'] !== 'admin' && req.headers['x-rol'] !== 'director') return res.status(403).json({ error: 'Sin permisos' });
    try { 
        await db.execute({ sql: `DELETE FROM gastos_socios WHERE id = ?`, args: [req.params.id] }); 
