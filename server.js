...
app.use(cors());
app.use(express.json({limit: '50mb'})); // Límite ampliado para subir imágenes
...
async function inicializarDB() {
    try {
        ...
        // NUEVA TABLA PARA TICKETS/FACTURAS ADJUNTAS
        ...
        console.log('✅ Base de datos Turso conectada y 100% operativa (Con módulo de Tickets).');
...
app.put('/api/usuarios/password', async (req, res) => {
    ...
    await db.execute({ sql: `UPDATE usuarios SET password = ? WHERE username = ?`, args: [req.body.newPass, req.body.username] }); res.json({ mensaje: 'Contraseña cambiada' });
});
...
app.post('/api/clientes', async (req, res) => {
    const rol = req.headers['x-rol']; const estado = rol === 'admin' ? 'APROBADO' : 'PENDIENTE'; const { nombre, nif, direccion, email, telefono, logo } = req.body;
    try { await db.execute({ sql: `INSERT INTO clientes (nombre, nif, direccion, email, telefono, logo, estado) VALUES (?, ?, ?, ?, ?, ?, ?)`, args: [nombre, nif, direccion, email, telefono, logo || '', estado] }); res.json({ mensaje: estado === 'APROBADO' ? 'Cliente añadido a la BBDD' : 'Petición enviada a Giancarlo para revisión.' }); } catch (e) { res.status(500).json({ error: 'Error al crear cliente' }); }
});
app.put('/api/clientes/:id/estado', async (req, res) => {
    ...
    try { await db.execute({ sql: `UPDATE clientes SET estado = ? WHERE id = ?`, args: [req.body.estado, req.params.id] }); res.json({ mensaje: `Cliente ${req.body.estado}` }); } catch (e) { res.status(500).json({ error: 'Error' }); }
});
...
app.post('/api/ot', async (req, res) => {
    ...
    try {
        ...
        await registrarLog(user, 'Añadir OT', `OT: ${datos.codigo_ot}`, datos, 'APROBADO'); res.json({ mensaje: 'OT guardada', id: Number(r.lastInsertRowid) });
...
app.put('/api/ot/:id/estado', async (req, res) => {
    ...
    if (rol === 'director') { await registrarLog(user, 'Editar OT', `Cambio estado OT ID: ${id}`, { id, nuevoEstado: estado }, 'PENDIENTE'); return res.json({ mensaje: 'Petición enviada a Giancarlo.' }); }
    try { await db.execute({ sql: `UPDATE ordenes_trabajo SET estado = ? WHERE id = ?`, args: [estado, id] }); await registrarLog(user, 'Editar OT', `Estado cambiado OT: ${id}`, { id, nuevoEstado: estado }, 'APROBADO'); res.json({ mensaje: 'Estado actualizado' }); } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/ot/:id', async (req, res) => {
    const rol = req.headers['x-rol']; const user = req.headers['x-user']; const { id } = req.params;
    if (rol === 'director') { await registrarLog(user, 'Eliminar OT', `Borrado OT ID: ${id}`, { id }, 'PENDIENTE'); return res.json({ mensaje: 'Petición enviada a Giancarlo.' }); }
    if (rol !== 'admin') return res.status(403).json({ error: 'Sin permisos.' });
    try { await db.execute({ sql: `DELETE FROM ordenes_trabajo WHERE id = ?`, args: [id] }); await registrarLog(user, 'Eliminar OT', `OT: ${id} eliminada`, { id }, 'APROBADO'); res.json({ mensaje: 'Eliminada' }); } catch (e) { res.status(500).json({ error: e.message }); }
});

// --- API TICKETS (ADJUNTOS) ---
app.get('/api/ot/:id/adjuntos', async (req, res) => {
    try { const r = await db.execute({ sql: "SELECT * FROM ot_adjuntos WHERE ot_id = ? ORDER BY id DESC", args: [req.params.id] }); res.json(r.rows); } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/ot/:id/adjuntos', async (req, res) => {
    ...
    try {
        ...
        res.json({ mensaje: 'Ticket guardado y materiales sumados.' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/factura', async (req, res) => {
    const { ot_id, codigo_ot, base_imponible, iva, total } = req.body; const fecha = new Date().toISOString().split('T')[0]; const txt = `NIF:B-26892760|FacturaRef:${codigo_ot}|Fecha:${fecha}|Total:${total}EUR`;
    try { const qr = await QRCode.toDataURL(txt); await db.execute({ sql: `INSERT INTO facturas (ot_id, base_imponible, iva, total, qr_data, fecha_emision) VALUES (?, ?, ?, ?, ?, ?)`, args: [ot_id, base_imponible, iva, total, qr, fecha] }); res.status(200).json({ mensaje: 'Factura emitida', qr_data: qr }); } catch (e) { res.status(500).json({ error: 'Error QR.' }); }
});
